...
lamda_em_specifier = ((lamda + suffix_em).add_action(merge) |
...
suffix_pl = ((pl + Optional(R("^[,;]$") + R("^[Mm]ax\w?$"))).add_action(merge) |
             (R("^[Mm]ax\w?$") + R("^[,;]$") + pl).add_action(merge) +
                ZeroOrMore(Not(W('nm')) + Any().hide()) + W("nm").hide())
tadf_pl_word = (I('photoluminescence') | W('PL') | I('fluorescence') | I('emission'))

lamda_pl_specifier = ((lamda + suffix_pl).add_action(merge) |
                      (R("^[λ𝛌𝜆𝝀𝝺𝞴](([Pp][Ll])|([Ee][Mm])|([Ff][Ll](uo)?))\w?$") + Optional(R("^[,;]$") + R("^[Mm]ax\w?$"))) |  #done
                      (R("^([λ𝛌𝜆𝝀𝝺𝞴]|(PL))[Mm]ax\w?$") + R("^[,;]$") + pl) |  # done
                      R("^[λ𝛌𝜆𝝀𝝺𝞴](([Pp][Ll])|([Ee][Mm])|([Ff][Ll](uo)?))?([Mm]ax)?\w?$") |  # done
                      ((I('emission') | I('emitting') | tadf_pl_word)  # done
                       + (I('wavelength') | R('^[pP]eaks?(ing)?$') | R('^[Mm]axim[(um)a]$'))) |  # done
                      (tadf_pl_word +  # done
                       Optional(R('^spectr[(um)a]$') | R('^intensit[(ies)y]')) +
                       SkipTo((R('^peaks?(ed)?$') | R('^cent[(ers?)(res?)]d?$') | W('around') | W("wavelength") | (R('^[Mm]axim[(um)a]$') + Optional(I('wavelength'))))).hide() +
                       (R('^peaks?(ed)?$') | R('^cent[(ers?)(res?)]d?$') | W("wavelength") | W('around') | (R('^[Mm]axim[(um)a]$') + Optional(I('wavelength'))))) |
...
class EmissionWavelength(LengthModel):
    ...
    """
    specifier = StringType(parse_expression=lamda_em_specifier,
                           required=True)
