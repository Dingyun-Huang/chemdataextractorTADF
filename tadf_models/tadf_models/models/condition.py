...
class Atmosphere(BaseModel):
    ...
    atmosphere = StringType(
        parse_expression=(I("air") | I("nitrogen") | W("N2") | I("argon") | I("inert")),
        required=True,
    )
...
names = importlib.resources.read_text(
    "tadf_models", "film_material_names", encoding="utf-8"
)
...
class Phase(BaseModel):
    ...
    specifier = StringType(
        parse_expression=(
            R("^[Ss]olutions?$")
            | (
                Optional(I("thin") + Optional(T("HYPH"))) + R("^[Ff]ilms?$")
                | R("^[hH]osts?$")
            )
        )
        .add_action(join)
    ...
    host = StringType(
        parse_expression=(solvent_name_options | First(film_materials)), required=False
    )
    parsers = [AutoTableParser(), AutoSentenceParser()]
...
temperature_specifier_expression = (
    I("temperature")
    | Every([R("^T\w?$"), Not(W("Tg")), Not(W("Td"))])
    | I("at")
    | I("near")
    | I("around")
    | I("above")
...
room_temperature_expression = (
    R("^r\.?t\.?$", re.I)
    | ((I("room") | I("ambient")) + Optional(I("-")) + (I("temperature") | I("temp")))
).add_action(join)
...
class TemperatureTableParser(AutoTableParser):
    ...
    def root(self):
        ...
        value_phrase = (
            value_element() + unit_element
        )  # Require that temperature value must be followed by the unit
...
class Temperature(TemperatureModel):
    ...
    specifier = StringType(
        parse_expression=temperature_specifier_expression,
        required=True,
        contextual=False,
        updatable=True,
    )
...
class RoomTemperature(BaseModel):
    ...
    context = ""
    room_temperature = StringType(
        parse_expression=room_temperature_expression, required=True
    )
    parsers = [AutoTableParser(), AutoSentenceParser()]
